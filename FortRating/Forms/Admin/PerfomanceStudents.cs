...
namespace FortRating.Forms.Admin
{
    public partial class PerfomanceStudents : Form
    {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < 1; i++)
                    {
                        ...
                        foreach (DataRow row in dt.Rows)
                        {
                            ...
                            listItems[i] = new StudentControl(of, row["idStudent"].ToString(), row["FIO"].ToString(), row["name"].ToString(), true, row["idGroup"].ToString());
        ...
        private void PerfomanceStudents_Load(object sender, EventArgs e)
        {
            calcPages();
            UsersPanel.Visible = false;
            guna2PictureBox1.Visible = true;
